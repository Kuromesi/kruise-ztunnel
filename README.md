...
Use Rust 1.90.0, matching [CI](.github/workflows/ci.yml), and install `protoc` before running `cargo build --locked`.
...
A release build with this option can be built with `cargo build --release --no-default-features --features tls-boring`.
